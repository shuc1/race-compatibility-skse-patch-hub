...
; 	Debug.Trace("C00: Player joined Companions.")
	...
	; RaceCompatibility: set player original race and apply cure if player is a vampire
...
; 	Debug.Trace("CRQ: Cycling radiant quests...")
	...
	if (Aela.GetActorReference().IsDead())
		if (AelaCurrentQuest != None)
			if AelaCurrentQuest.IsRunning() && AelaCurrentQuest.GetStage() < 255
		...
		if (AelaNextQuest != None)
			if AelaNextQuest.IsRunning() && AelaNextQuest.GetStage() < 255
	...
	if (Farkas.GetActorReference().IsDead())
		if (FarkasCurrentQuest != None)
			if FarkasCurrentQuest.IsRunning() && FarkasCurrentQuest.GetStage() < 255
		...
		if (FarkasNextQuest != None)
			if FarkasNextQuest.IsRunning() && FarkasNextQuest.GetStage() < 255
	...
	if (Vilkas.GetActorReference().IsDead())
		if (VilkasCurrentQuest != None)
			if VilkasCurrentQuest.IsRunning() && VilkasCurrentQuest.GetStage() < 255
		...
		if (VilkasNextQuest != None)
			if VilkasNextQuest.IsRunning() && VilkasNextQuest.GetStage() < 255
	...
	if (Skjor.GetActorReference().IsDead())
		if (SkjorCurrentQuest != None)
			if SkjorCurrentQuest.IsRunning() && SkjorCurrentQuest.GetStage() < 255
		...
		if (SkjorNextQuest != None)
			if SkjorNextQuest.IsRunning() && SkjorNextQuest.GetStage() < 255
